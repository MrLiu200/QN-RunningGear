...
Bus1API *Bus1API::self = 0;
Bus1API *Bus1API::Instance()
{
    if(!self){
        QMutex mutex;
...
Bus1API::Bus1API(QObject *parent) : QObject(parent)
{
    IsOpen = false;
    currentpreID = 0;
    currentpreCH = 0;
...
bool Bus1API::Open()
{
    ...
    //打开硬件上的CAN设备
    QProcess process;
    QString SYStemCmdDownCan = QString("ifconfig %1 down").arg(APPSetting::can1_Name);
    ...
    QString SYStemCmdconf = QString("ip link set %1 up type can bitrate %2 sample-point %3 dbitrate %4 dsample-point %5 fd on")
            .arg(APPSetting::can1_Name).arg(APPSetting::can1_bitrate)
            .arg(APPSetting::can1_bitrateSample).arg(APPSetting::can1_dbitrate).arg(APPSetting::can1_dbitrateSample);
    process.start(SYStemCmdconf);
    process.waitForFinished(-1);
#endif
    //创建can设备
    QString errorString;
    m_canDevice.reset(QCanBus::instance()->createDevice(APPSetting::CanPlugin, APPSetting::can1_Name,&errorString));
    if (!m_canDevice) {
        qWarning() << QString("Failed to create CAN1 device,reason:%1").arg(errorString);
        IsOpen = false;
    }else{
        IsOpen = true;
    }
    ...
    if(IsOpen){
        connect(m_canDevice.get(), &QCanBusDevice::errorOccurred,this, &Bus1API::processErrors);
        connect(m_canDevice.get(), &QCanBusDevice::framesReceived,this, &Bus1API::processReceivedFrames);

        /*设置can参数*/
        //仲裁域波特率
        m_canDevice->setConfigurationParameter(QCanBusDevice::BitRateKey,QVariant());
        //开启canfd模式
        if(APPSetting::can1_Mode == "canfd"){
            m_canDevice->setConfigurationParameter(QCanBusDevice::CanFdKey,QVariant(true));
        }else if(APPSetting::can1_Mode == "can"){
        ...
        if (!m_canDevice->connectDevice()) {
            qWarning() << "Failed to connect to CAN1 device:" << m_canDevice->errorString();
            m_canDevice.reset();
            IsOpen = false;
        }
    }
...
void Bus1API::processReceivedFrames()
{
    if (!IsOpen)
        return;
...
void Bus1API::processErrors(QCanBusDevice::CanBusError error) const
{
    switch (error) {
    ...
    case QCanBusDevice::UnknownError:
        qDebug()<<"Bus1API Error: " + m_canDevice->errorString();
        break;
    default:
        break;
    }
...
void Bus1API::checkData(QByteArray frameData, quint32 frameID)
{
...
//                qDebug()<<"bus1 checkData "  << frameID  << ": VibdataArr.size() = " << VIBDataLen;
                //查找该通道灵敏度
                ...
                if(!DBData::SpeedStruct.Bus1SpeedVector.isEmpty()){
                    vibration_data.SpeedVector = DBData::SpeedStruct.Bus1SpeedVector;
                }
...
//                                                                Raw_Max,Rawave,Demave);
            ...
            qDebug()<<"frameID = " << frameID << "currentID = " << currentpreID;
        }
    }else{//解析命令
        qDebug()<<"bus1 checkData "  << frameID  << ": "<< CoreHelper::byteArrayToHexStr(frameData);
        uint8_t frame[len];
        //根据数据长度，截取正确数据包
...
//                UpdateDeviceState(frameID,0);
            ...
            case 0x03://振动数据采集完成
            {
                DBData::SpeedStruct.Bus1CanSave = false;
                ...
                //                int CH_Count = frame[PRE_PACKET_DATA_OFFSET + 2];
                ...
                this->VIBDataLen = frame[len-5] << 24 | frame[len-4] << 16 | frame[len-3] << 8 | frame[len-2];
                //温度信息存入链表
                //环境温度、通道1、2、3、4、5…… 温度
...
QByteArray Bus1API::GetSendPackage(uint8_t pre_id, uint8_t order, QByteArray data)
{
    QByteArray array;
    array.append(pre_id);
    array.append(order);
    ...
    qDebug()<<"can1 send :" << CoreHelper::byteArrayToHexStr(array);
    return array;
}

void Bus1API::SendData(const QCanBusFrame &frame)
{
    if(!m_canDevice) return;
    m_canDevice->writeFrame(frame);
}

double Bus1API::findTemperatures(uint8_t preid, uint8_t prech)
{
    double ret = 0.0;
...
void Bus1API::addTemperatures(uint8_t preid, uint8_t prech, double value)
{
    // 遍历列表查找是否存在符合条件的元素
    bool found = false;
...
void Bus1API::addTaskprelist(uint8_t preid, uint8_t channelsNumber,QVector<uint8_t> CH_Status)
{
    // 检查 channelsNumber 是否有效或者传入参数不匹配
       ...
       if(!Bus1PreList.contains(preid)){
           Bus1PreList.append(preid);
       }
            ...
            if(DBData::DeviceInfo_IsEnable.at(deviceindex)){
                if(CH_Status.at(i-1) == 0){
                    ...
                    if(channellist.working_ch == 0){
                     ...
                     qDebug()<<"can 1 add Task" << preid << ": channel = " << i;
                }
            }
        }
    }
//    qDebug()<<"can 1 add Task" << preid << ": channelsNumber = " << channelsNumber;
...
void Bus1API::UpdateDeviceState(uint8_t preid, uint8_t prech, bool IsFault)
{
...
void Bus1API::CommandTemInspection(uint8_t PreID)
{
    QCanBusFrame frame;
    frame.setFrameId(CAN_Default_FrameID);
...
void Bus1API::CommandVibInspection(uint8_t PreID, uint8_t PreCH)
{
    QByteArray array;
    array.append(PreCH);
    ...
    SendData(frame);
    SetTaskState(PreID,TASKSTATE::Collecting);
    DBData::SpeedStruct.Bus1SpeedVector.clear();
    DBData::SpeedStruct.Bus1CanSave = true;
}

void Bus1API::CommandReadVibData(uint8_t PreID, uint8_t PreCH)
{
    QByteArray array;
    array.append(PreCH);
...
void Bus1API::CommandPreUpdate(uint8_t PreID, uint32_t crc, int len)
{
    QByteArray array;
...
void Bus1API::CommandSendFile(QString filename)
{
    QFile file(filename);
...
void Bus1API::CommandReboot(uint8_t PreID)
{
    QCanBusFrame frame;
    frame.setFrameId(CAN_Default_FrameID);
...
QByteArray Bus1API::GetTaskState()
{
...
QByteArray Bus1API::switchNextTask()
{
    QByteArray array;
    if (task_pre_list.prelist.isEmpty()) {
...
void Bus1API::switchNextChannel(uint8_t preid)
{
    if (task_pre_list.prelist.isEmpty()) {
        qWarning() << "No tasks in the prelist.";
        return; // 直接返回
    ...
    for (auto &task : task_pre_list.prelist) { // 遍历使用引用
        if (task.Pre_id == preid && task.status == TASKSTATE::WaitingSwitch) { //只有等待切换才可以切换通道
            ...
            qDebug()<<"bus1 switchNextChannel" << "working_id = " << task.Pre_id << "working_ch = " << task.working_ch << "new state = "
                   << task.status;
            break;
#else
            //增加通道是否故障判断
            ...
            for (int i = 0; i < task.channellist.size(); ++i) {
                int currentIndex = (startIndex + i) % task.channellist.size();
...
void Bus1API::SetTaskState(uint8_t preid, TASKSTATE state)
{
    if (task_pre_list.prelist.isEmpty()) {
        qWarning() << "No tasks in the prelist.";
        return; // 直接返回
...
bool Bus1API::TaskIsContain(uint8_t preid)
{
#if 0
    ...
    return false;
#else
    return Bus1PreList.contains(preid);
#endif
}

QList<uint8_t> Bus1API::GetPreDeviceList()
{
...
bool Bus1API::BusIsOpen()
{
    return this->IsOpen;
}

void Bus1API::LowPowerMode()
{
...
void Bus1API::QueryCHState(uint8_t id, uint8_t ch)
{
    QByteArray array;
    array.append(ch);
